...
polyvec_compress :: proc "contextless" (r: []byte, a: ^Polyvec, kay: int) #no_bounds_check {
	...
	r := r
	switch len(r) {
	case ML_KEM_POLYVECCOMPRESSEDBYTES_768:
		ensure_contextless(kay == ML_KEM_K_768)

		t: [4]u16
		defer crypto.zero_explicit(&t, size_of(t))

		for i in 0..<ML_KEM_K_768 {
			for j in 0..<ML_KEM_N/4 {
				for k in 0..<4 {
					t[k] = u16(a.vec[i].coeffs[4*j+k])
					t[k] += u16((i16(t[k]) >> 15) & ML_KEM_Q)
					// t[k]  = ((((uint32_t)t[k] << 10) + ML_KEM_Q/2)/ML_KEM_Q) & 0x3ff
					d0 = u64(t[k])
					d0 <<= 10
					d0 += 1665
					d0 *= 1290167
	...
	case ML_KEM_POLYVECCOMPRESSEDBYTES_1024:
		ensure_contextless(kay == ML_KEM_K_1024)

		t: [8]u16
		defer crypto.zero_explicit(&t, size_of(t))

		for i in 0..<ML_KEM_K_1024 {
			for j in 0..<ML_KEM_N/8 {
				for k in 0..<8 {
					t[k] = u16(a.vec[i].coeffs[8*j+k])
					t[k] += u16((i16(t[k]) >> 15) & ML_KEM_Q)
					// t[k]  = ((((uint32_t)t[k] << 11) + ML_KEM_Q/2)/ML_KEM_Q) & 0x7ff
					d0 = u64(t[k])
					d0 <<= 11
					d0 += 1664
					d0 *= 645084
	...
	case:
		panic_contextless("crypto/mlkem: invalid ML_KEM_POLYVECCOMPRESSEDBYTES")
	}
}

polyvec_decompress :: proc "contextless" (r: ^Polyvec, a: []byte) #no_bounds_check {
	a := a
	switch len(a) {
	case ML_KEM_POLYVECCOMPRESSEDBYTES_768:
		t: [4]u16
		...
		for i in 0..<ML_KEM_K_768 {
			for j in 0..<ML_KEM_N/4 {
				...
				for k in 0..<4 {
					r.vec[i].coeffs[4*j+k] = i16((u32(t[k] & 0x3FF) * ML_KEM_Q + 512) >> 10)
				}
			}
		}
	case ML_KEM_POLYVECCOMPRESSEDBYTES_1024:
		...
		for i in 0..<ML_KEM_K_1024 {
			for j in 0..<ML_KEM_N/8 {
				...
				for k in 0..<8 {
					r.vec[i].coeffs[8*j+k] = i16((u32(t[k] & 0x7FF) * ML_KEM_Q + 1024) >> 11)
				}
			}
		}
	case:
...
polyvec_tobytes :: proc "contextless" (r: []byte, a: ^Polyvec, k: int) #no_bounds_check {
	...
	for i in 0..<k {
		...
		r = r[ML_KEM_POLYBYTES:]
	}
}

polyvec_frombytes :: proc "contextless" (r: ^Polyvec, a: []byte, k: int) #no_bounds_check {
	switch k {
	case ML_KEM_K_768:
	case ML_KEM_POLYVECBYTES_768, ML_KEM_POLYVECBYTES_1024:
	case:
		panic_contextless("crypto/mlkem: invalid ML_KEM_POLYVECBYTES")
	}
	ensure_contextless(len(a) == k * ML_KEM_POLYBYTES, "crypto/mlkem: invalid buffer")

	a := a
	for i in 0..<k {
		poly_frombytes(&r.vec[i], a)
		a = a[ML_KEM_POLYBYTES:]
	}
}
...
polyvec_byte_size :: #force_inline proc "contextless" (k: int) -> int {
	switch k {
	case ML_KEM_K_768, ML_KEM_K_1024:
		return k * ML_KEM_POLYBYTES
	case:
		return 0
	}
}
