...
poly_compress :: proc "contextless" (r: []byte, a: ^Poly) #no_bounds_check {
	...
	r := r
	switch len(r) {
	case ML_KEM_POLYCOMPRESSEDBYTES_768:
		for i in 0..<ML_KEM_N/8 {
			for j in 0..<8 {
				// map to positive standard representatives
				u := a.coeffs[8*i+j]
				u += (u >> 15) & ML_KEM_Q
				// t[j] = ((((uint16_t)u << 4) + ML_KEM_Q/2)/ML_KEM_Q) & 15
				d0 := u32(u) << 4
				d0 += 1665
				d0 *= 80635
				d0 >>= 28
	...
	case ML_KEM_POLYCOMPRESSEDBYTES_1024:
		for i in 0..<ML_KEM_N/8 {
			for j in 0..<8 {
				// map to positive standard representatives
				u := a.coeffs[8*i+j]
				u += (u >> 15) & ML_KEM_Q
				// t[j] = ((((uint16_t)u << 5) + ML_KEM_Q/2)/ML_KEM_Q) & 31
				d0 := u32(u) << 5
				d0 += 1664
				d0 *= 40318
				d0 >>= 27
	...
	case:
		panic_contextless("crypto/mlkem: invalid ML_KEM_POLYCOMPRESSEDBYTES")
	}
}
...
poly_decompress :: proc "contextless" (r: ^Poly, a: []byte) {
	...
	switch len(a) {
	case ML_KEM_POLYCOMPRESSEDBYTES_768:
		for i in 0..<ML_KEM_N/2 {
			r.coeffs[2*i+0] = i16(((u16(a[0] & 15) * ML_KEM_Q) + 8) >> 4)
			r.coeffs[2*i+1] = i16(((u16(a[0] >> 4) * ML_KEM_Q) + 8) >> 4)
			a = a[1:]
		}
	case ML_KEM_POLYCOMPRESSEDBYTES_1024:
		t: [8]byte = ---
		...
		for i in 0..<ML_KEM_N/8 {
			...
			for j in 0..<8 {
				r.coeffs[8*i+j] = i16((u32(t[j] & 31) * ML_KEM_Q + 16) >> 5)
			}
		}
	case:
		panic_contextless("crypto/mlkem: invalid ML_KEM_POLYCOMPRESSEDBYTES")
...
poly_tobytes :: proc "contextless" (r: []byte, a: ^Poly) #no_bounds_check {
	ensure_contextless(len(r) >= ML_KEM_POLYBYTES)

	for i in 0..<ML_KEM_N/2 {
		// map to positive standard representatives
		t0 := u16(a.coeffs[2*i])
		t0 += u16((i16(t0) >> 15) & ML_KEM_Q)
		t1 := u16(a.coeffs[2*i+1])
		t1 += u16((i16(t1) >> 15) & ML_KEM_Q)
		r[3*i+0] = byte(t0 >> 0)
...
poly_frombytes :: proc "contextless" (r: ^Poly, a: []byte) #no_bounds_check {
	ensure_contextless(len(a) >= ML_KEM_POLYBYTES)
...
poly_frommsg :: proc "contextless" (r: ^Poly, msg: []byte) #no_bounds_check {
	#assert(ML_KEM_INDCPA_MSGBYTES == ML_KEM_N/8)
	ensure_contextless(len(msg) == ML_KEM_INDCPA_MSGBYTES)

	for i in 0..<ML_KEM_N/8 {
		for j in 0..<8 {
			r.coeffs[8*i+j] = subtle.csel_i16(0, (ML_KEM_Q+1)/2, int(msg[i] >> uint(j))&1)
		}
	}
}
...
poly_getnoise_eta1 :: proc(r: ^Poly, seed: []byte, iv: byte) {
	buf: [ML_KEM_ETA1*ML_KEM_N/4]byte = ---
...
poly_getnoise_eta2 :: proc(r: ^Poly, seed: []byte, iv: byte) {
	buf: [ML_KEM_ETA2*ML_KEM_N/4]byte = ---
	defer crypto.zero_explicit(&buf, size_of(buf))
...
poly_basemul_montgomery :: proc "contextless" (r, a, b: ^Poly) #no_bounds_check {
	for i in 0..<ML_KEM_N/4 {
		j := 4 * i
		r.coeffs[j], r.coeffs[j+1] = base_case_multiply(a.coeffs[j], a.coeffs[j+1], b.coeffs[j], b.coeffs[j+1], ZETAS[64+1])
		r.coeffs[j+2], r.coeffs[j+3] = base_case_multiply(a.coeffs[j+2], a.coeffs[j+3], b.coeffs[j+2], b.coeffs[j+3], -ZETAS[64+1])
	}
}
...
poly_compressed_bytes :: #force_inline proc "contextless" (k: int) -> int {
	switch k {
	case ML_KEM_K_768:
		return ML_KEM_POLYCOMPRESSEDBYTES_768
	case ML_KEM_K_1024:
		return ML_KEM_POLYCOMPRESSEDBYTES_1024
	case:
