...
pack_pk :: proc "contextless" (r: []byte, pk: ^Polyvec, seed: []byte, k: int) -> bool {
	...
	case pk_len == 0:
		return false
	case len(seed) != ML_KEM_SYMBYTES || len(r) != pk_len + ML_KEM_SYMBYTES:
		return false
	}
...
unpack_pk :: proc "contextless" (pk: ^Polyvec, seed, packedpk: []byte) -> bool {
	pk_len := len(packedpk) - ML_KEM_SYMBYTES
	k: int
	switch {
	case pk_len == ML_KEM_POLYVECBYTES_768:
		k = ML_KEM_K_768
	case pk_len == ML_KEM_POLYVECBYTES_1024:
		k = ML_KEM_K_1024
	case len(packedpk) - pk_len != ML_KEM_SYMBYTES:
		return false
	case len(seed) != ML_KEM_SYMBYTES:
		return false
	}
	if k == 0 {
		return false
	...
	polyvec_frombytes(pk, packedpk[:pk_len], k)
...
unpack_sk :: proc "contextless" (sk: ^Polyvec, packedsk: []byte) -> bool {
	k: int
	switch len(packedsk) {
	case ML_KEM_POLYVECBYTES_768:
		k = ML_KEM_K_768
	case ML_KEM_POLYVECBYTES_1024:
		k = ML_KEM_K_1024
	case:
		return false
	}
	...
	polyvec_frombytes(sk, packedsk, k)
...
unpack_ciphertext :: proc "contextless" (b: ^Polyvec, v: ^Poly, c: []byte) -> int {
	...
	k: int
	switch len(c) {
	case ML_KEM_POLYVECCOMPRESSEDBYTES_768 + ML_KEM_POLYCOMPRESSEDBYTES_768:
		b_len = ML_KEM_POLYVECCOMPRESSEDBYTES_768
		k = ML_KEM_K_768
	case ML_KEM_POLYVECCOMPRESSEDBYTES_1024 + ML_KEM_POLYCOMPRESSEDBYTES_1024:
		b_len = ML_KEM_POLYVECCOMPRESSEDBYTES_1024
		k = ML_KEM_K_1024
	case:
		return 0
	}

	polyvec_decompress(b, c[:b_len])
...
gen_matrix :: proc(a: []Polyvec, seed: []byte, transposed: bool, k: int) {
	GEN_MATRIX_NBLOCKS :: ((12*ML_KEM_N/8*(1 << 12)/ML_KEM_Q + XOF_BLOCKBYTES)/XOF_BLOCKBYTES)
	...
	for i in 0..<k {
		for j in 0..<k {
			...
			shake.read(&ctx, buf[:])
			ctr = rej_uniform(a[i].vec[j].coeffs[ctr:], buf[:])
...
k_pke_keygen :: proc(
	...
	assert(len(d) == ML_KEM_SYMBYTES, "crypto/mlkem: invalid K-PKE d")
	ensure(k == ML_KEM_K_768 || k == ML_KEM_K_1024, "crypto/mlkem: invalid k")

	buf: [2*ML_KEM_SYMBYTES]byte
	defer crypto.zero_explicit(&buf, size_of(buf))

	a_: [ML_KEM_K_MAX]Polyvec
	e: Polyvec
	...
	copy(buf[:], d)
	buf[ML_KEM_SYMBYTES] = byte(k)
	hash_g(buf[:], buf[:ML_KEM_SYMBYTES+1])

	p, sigma := buf[:ML_KEM_SYMBYTES], buf[ML_KEM_SYMBYTES:]
	...
	for i in 0..<k {
		poly_getnoise_eta2(&e.vec[i], sigma, n)
		n += 1
	}
...
k_pke_encrypt :: proc(
	...
	ensure(len(m) == ML_KEM_INDCPA_MSGBYTES, "crypto/mlkem: invalid K-PKE m")
	ensure(len(r) == ML_KEM_SYMBYTES, "crypto/mlkem: invalid K-PKE r")

	k := ek.k

	at_: [ML_KEM_K_MAX]Polyvec
	sp, ep, b: Polyvec
	kay, epp, v: Poly
...
k_pke_decrypt :: proc(
	...
	if len(plaintext) != ML_KEM_INDCPA_MSGBYTES {
		return false
	}
